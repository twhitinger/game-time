...
DirectionSpinner.prototype.eventListen = function(ball,directionSpinner,once, powerBar) {
  window.addEventListener("keyup", spinnerTurn);

  function spinnerTurn(e) {
    if (e.keyCode === 32) {
      once += 1;
      directionTurn(ball,directionSpinner,once,powerBar);
    }
  }
  function directionTurn(ball,directionSpinner,once,powerBar) {
    ball.radian = directionSpinner.pointerRadians;
    directionSpinner.direction = ball.radian;
    directionSpinner.resistance = 0.15;
...
DirectionSpinner.prototype.directionTurn = function(ball,directionSpinner, powerBar) {
  ball.radian = directionSpinner.pointerRadians;
